...
export function FinalCTA({ onOpenModal }: FinalCTAProps) {
  ...
  return (
    <section ref={sectionRef} className="py-10 md:py-14 bg-[#FAFAFA] relative overflow-hidden" id="get-started">
      <div className="max-w-[1350px] mx-auto px-4 sm:px-6 md:px-12 relative z-10">
        
        {/* FULL-WIDTH PREMIUM CANVAS CARD */}
        <div className="bg-gradient-to-b from-[#1E3A8A] via-[#2563EB] to-[#1D4ED8] rounded-[32px] md:rounded-[36px] p-6 sm:p-10 md:p-12 lg:p-14 text-white shadow-2xl relative overflow-hidden border border-blue-400/30">
          ...
          <div className="text-center max-w-3xl mx-auto mb-10 md:mb-12 relative z-10">
            ...
            <motion.div 
              ...
              className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-white shadow-xs mb-4"
            >
              <motion.span 
                animate={{ y: [0, -3, 0] }}
            ...
            <motion.h2 
              ...
              className="text-3xl md:text-5xl lg:text-6xl font-black font-sans tracking-tight leading-[1.12] mb-4 text-white"
            >
            ...
            <motion.p 
              ...
              className="text-base md:text-lg text-blue-100 leading-relaxed max-w-2xl mx-auto font-medium"
            >
          ...
          <motion.div 
            ...
            className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10 md:mb-12 relative z-10 items-stretch"
          >

            {/* CARD 1: ₹499 / MONTH (FEATURED & HIGHLIGHTED AS MOST POPULAR) */}
            <motion.div 
            ...
            {/* CARD 2: ₹4,999 ONE-TIME (FULL OWNERSHIP BADGE) */}
            <motion.div 
              variants={itemVariants}
            ...
            >
              <div>
                ...
                <p className="text-[10px] text-blue-200 bg-white/10 p-2.5 rounded-xl border border-white/15 mb-6 leading-normal font-medium">
                  Note: Future updates, maintenance, and new features are available at an additional cost whenever needed.
                </p>
              </div>
          ...
          <motion.div 
            ...
            initial="hidden"
            animate={isInView ? "show" : "hidden"}
            className="flex flex-wrap items-center justify-center gap-2.5 md:gap-3 mb-10 relative z-10"
          >
            {trustBadges.map((badge, idx) => (
              <motion.div
                ...
                className="bg-white/10 backdrop-blur-md border border-white/15 px-3.5 py-1.5 rounded-2xl text-[11px] font-bold text-white shadow-2xs"
              >
          ...
          {/* HIGH-QUALITY REALISTIC SOCIAL PROOF AVATARS STACK */}
          <motion.div 
            initial={{ opacity: 0, y: 10 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.5, delay: 0.5 }}
            className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-8 relative z-10 text-center"
          >
          ...
          <motion.div 
            ...
            className="flex flex-col sm:flex-row items-center justify-center gap-3.5 max-w-md mx-auto relative z-10"
          >
            <motion.button 
              onClick={() => onOpenModal?.('care')}
              whileHover={{ scale: 1.03 }} 
              whileTap={{ scale: 0.97 }}
              className="w-full sm:w-auto bg-white text-[#2563EB] px-7 py-3.5 rounded-2xl font-black text-xs sm:text-sm shadow-xl hover:bg-blue-50 transition-all flex items-center justify-center gap-2 group text-center cursor-pointer"
            >
            ...
            <motion.a 
              ...
              whileHover={{ scale: 1.03 }} 
              whileTap={{ scale: 0.97 }}
              className="w-full sm:w-auto bg-emerald-500 text-white border border-emerald-400/30 px-7 py-3.5 rounded-2xl font-bold text-xs sm:text-sm shadow-lg hover:bg-emerald-600 transition-all flex items-center justify-center gap-2 text-center cursor-pointer"
            >
