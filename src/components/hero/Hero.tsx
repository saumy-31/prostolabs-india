...
export function Hero({ onOpenModal }: HeroProps) {
  ...
  return (
    <section 
      ...
      className="relative pt-20 sm:pt-24 lg:pt-28 pb-6 sm:pb-8 lg:pb-12 bg-[#FAFAFA] overflow-hidden select-none" 
      id="hero"
    >
      ...
      <motion.div 
        ...
        className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] sm:w-[1000px] h-[350px] sm:h-[550px] bg-[#2563EB] rounded-full blur-[100px] sm:blur-[120px] pointer-events-none" 
      />

      {/* CONTAINER WITH GUARANTEED 20-24PX MARGINS ON MOBILE */}
      <div className="max-w-[1300px] mx-auto px-5 sm:px-6 md:px-12 relative z-10">
        
        {/* GRID STRUCTURE: COMPACT SPACING ON MOBILE */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-3 sm:space-y-4 lg:space-y-5">
            ...
            >
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 sm:px-3.5 sm:py-1.5 rounded-full bg-blue-50 border border-blue-100/90 shadow-2xs">
            ...
            {/* HEADLINE: SCALED FOR MOBILE (36px–40px) */}
            <h1 className="text-[2.15rem] leading-[1.1] sm:text-5xl lg:text-6xl font-black text-[#0A0A0A] font-sans tracking-tight">
              {/* Line 1 */}
            ...
            {/* DESCRIPTION: SLIGHTLY REDUCED PARAGRAPH FONT & WIDTH ON MOBILE */}
            <motion.p 
            ...
            {/* PRICING COMPARISON STRIP: COMPACT MOBILE CARD HEIGHT */}
            <motion.div 
            ...
            {/* CTA BUTTON: FULL WIDTH 52px ON MOBILE */}
            <div className="pt-1 flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3">
              <motion.button 
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full sm:w-auto h-[52px] sm:h-auto bg-[#2563EB] text-white font-bold text-xs sm:text-sm px-6 sm:px-7 py-3.5 rounded-xl shadow-md sm:shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all flex items-center justify-center gap-2 group text-center cursor-pointer"
              >
            ...
            {/* TRUST BADGES: CLEAN 2-COLUMN GRID ON MOBILE (FULLY ALIGNED) */}
            <div className="pt-2 sm:pt-3 border-t border-gray-200/60">
              <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-x-3 gap-y-2 sm:gap-x-5 text-[11px] sm:text-xs font-semibold text-gray-600">
                {[
          ...
          {/* RIGHT COLUMN: BROWSER PREVIEW (REDUCED SIZE BY ~25% ON MOBILE) */}
          <motion.div 
            style={{ y: mockupScrollY }}
            className="lg:col-span-5 relative perspective-1000 mt-2 sm:mt-4 lg:mt-0 max-w-[340px] xs:max-w-[380px] sm:max-w-none mx-auto lg:mx-0 transform scale-90 sm:scale-100 origin-top"
          >
            <div className="relative rounded-xl sm:rounded-2xl overflow-hidden shadow-lg sm:shadow-2xl border border-gray-200/80 bg-white">
