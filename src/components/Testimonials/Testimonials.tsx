...
export function Testimonials() {
  ...
  return (
    <section ref={sectionRef} className="py-12 sm:py-20 md:py-32 bg-[#FAFAFA] relative overflow-hidden" id="testimonials">
      ...
      <div className="max-w-[1300px] mx-auto px-4 sm:px-6 md:px-12 relative z-10">
        ...
        >
          <div className="bg-white border border-gray-200/90 rounded-xl sm:rounded-2xl px-3.5 sm:px-5 py-2 sm:py-3 shadow-2xs flex flex-wrap items-center justify-center gap-2 sm:gap-3">
            ...
            <span className="text-[10px] sm:text-xs text-gray-500 font-medium hidden xs:inline">Verified Local Business Reviews</span>
          </div>
        </motion.div>

        {/* INTERACTIVE SHOWCASE STAGE */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-start mb-12 sm:mb-20">
          ...
          <div className="lg:col-span-7">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="bg-white border-2 border-[#2563EB]/80 rounded-2xl sm:rounded-[32px] p-5 sm:p-8 md:p-10 shadow-lg sm:shadow-xl relative overflow-hidden flex flex-col justify-between min-h-0 sm:min-h-[440px]"
              >
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-4 sm:mb-8">
                    <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-blue-50 text-[#2563EB] border border-blue-100 text-[10px] sm:text-xs font-bold uppercase tracking-wider">
                  ...
                  <div className="relative mb-5 sm:mb-8">
                    <Quote className="text-blue-100/80 w-8 h-8 sm:w-12 sm:h-12 absolute -top-3 -left-1 sm:-top-4 sm:-left-2 -z-10" />
                    <p className="text-base sm:text-xl md:text-2xl font-bold text-[#0A0A0A] leading-snug font-sans tracking-tight">
                      "{current.quote}"
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-4 sm:pt-6 border-t border-gray-100 flex flex-row items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2.5 sm:gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] sm:text-xs text-gray-500 font-medium truncate max-w-[150px] xs:max-w-[200px] sm:max-w-none">{current.role}, {current.business}</p>
                      <p className="text-[10px] sm:text-[11px] text-gray-400 flex items-center gap-1 mt-0.5">
                        <MapPin size={10} /> {current.location}
                      </p>
                    </div>
                  </div>

                  <div className="bg-gray-50 border border-gray-200/80 rounded-lg sm:rounded-xl px-2.5 py-1.5 sm:px-3.5 sm:py-2 text-right shrink-0">
                    <div className="text-[9px] sm:text-[10px] font-bold text-gray-400 uppercase tracking-wider">Plan</div>
                    <div className="text-[11px] sm:text-xs font-bold text-[#2563EB]">{current.packageSelected}</div>
                  </div>
          ...
          {/* RIGHT SIDE: VERTICAL STACK SELECTOR (SWIPEABLE STRIP ON MOBILE) */}
          <div className="lg:col-span-5 flex lg:flex-col gap-2.5 sm:gap-3 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
            {testimonials.map((item, idx) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={{ x: 4 }}
                  className={`text-left p-3 sm:p-4 rounded-xl sm:rounded-2xl border transition-all duration-300 flex items-center justify-between gap-3 shrink-0 min-w-[220px] sm:min-w-[280px] lg:min-w-0 cursor-pointer ${
                    isActive 
                ...
                >
                  <div className="flex items-center gap-2.5 sm:gap-3">
                    <div className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl shrink-0 ${isActive ? 'bg-[#2563EB] text-white' : 'bg-gray-100 text-gray-600'}`}>
                      <IconComp size={16} />
                    </div>
        ...
        <motion.div 
          ...
          className="pt-6 sm:pt-10 border-t border-gray-200/80"
        >
          <p className="text-center text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 sm:mb-8">
            ...
            {businessLogos.map((logo, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white border border-gray-200/80 rounded-xl sm:rounded-2xl p-2.5 sm:p-4 flex flex-col items-center justify-center gap-1.5 shadow-2xs transition-all duration-200"
                >
                  <LogoIcon className="w-4 h-4 sm:w-5 sm:h-5 text-[#2563EB]" />
                  <span className="text-[11px] sm:text-xs font-bold text-[#0A0A0A] font-sans text-center truncate w-full">{logo.name}</span>
                  <span className="text-[9px] sm:text-[10px] text-gray-400 font-medium hidden xs:block">{logo.category}</span>
                </motion.div>
              )
            })}
          </div>
